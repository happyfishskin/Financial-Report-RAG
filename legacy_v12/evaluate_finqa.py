...
def build_prompt(context: str, question: str) -> str:
    """
    CoT 提示詞（強化文字 Schema 約束版）。
    不依賴後端 guided_json 參數，透過提示詞本身硬性規定輸出格式，
    確保在任何 OpenAI 相容後端（vLLM / Ollama / OpenAI）均能解析。
    核心約束：
    1. thought：簡短計算過程，幫助模型正確推理。
    2. answer：純數值或 yes/no，禁止附加單位或百分號。
    3. 百分比結果必須轉為小數（8.61% → 0.0861）。
    """
    return (
        f"Context:\n{context}\n\n"
        f"Question: {question}\n\n"
...
def normalize_number(s: str) -> Optional[float]:
    """
    將字串轉換為 float，處理以下常見格式：
    - LaTeX 跳脫百分號：14.46\\% → 14.46%（先清洗再處理）
    - 千分位逗號：1,234.56 → 1234.56
    - 百分比：14.46% → 0.1446（自動除以 100）
    - 一般小數與整數
    """
    if s is None:
        return None
    # 清洗 LaTeX \% 跳脫與多餘空格，再去除千分位逗號
...
def is_correct(
    ...
    """
    統一答案評估入口，依據 Ground Truth 類型選擇比對策略：

    ① 是非題（truth 為 "yes" / "no"）：
       直接進行字串不分大小寫比對。

    ② 數值題（依序嘗試以下匹配層）：
       a. 標準容差：絕對誤差 ≤ 0.01，或相對誤差 ≤ 5%
       b. 百分比 ↔ 小數換算：pred÷100 或 pred×100 後再比對
       c. 語意智慧捕獲：pred > 1 且 GT < 1 時，若 raw_response
          末尾 100 字元含「%/percent/percentage」，自動補除以 100
       d. 正負號容差：財務「下降幅度」正負號歧義（如 -34.79 vs 0.34807），
          若 abs(pred) 或 abs(pred/100) 與 abs(GT) 在容差內，視為正確

    回傳 (is_correct: bool, parse_failed: bool)
    """
    if pred is None:
        return False, True
    ...
    def within_tolerance(a: float, b: float) -> bool:
        abs_diff = abs(a - b)
        # 絕對誤差：覆蓋趨近 0 的小數（如 0.0009）及整數捨入
        if abs_diff <= 0.01:
            return True
        # 相對誤差：放寬至 5%，相容 4B AWQ 在長輸出截斷邊緣的精度損失
    ...
    # b. 百分比 ↔ 小數換算保險層
    #    覆蓋 GT 以整數百分比儲存（如 truth="14.464" 而非 "0.14464"）的情況
    if within_tolerance(pred_val / 100.0, truth_val):
        return True, False
    if within_tolerance(pred_val * 100.0, truth_val):
        return True, False

    # c. 語意智慧捕獲：b 層標準容差（5%）未命中，但 pred > 1、GT < 1，
    #    且 raw_response 末尾 100 字含百分比關鍵字，啟用寬容差（10%）再試一次。
    #    典型場景：模型在 \boxed{} 忘記寫 %，且計算本身存在輕微誤差
    #    （差距介於 5%–10% 之間），憑語意閘門提供額外的寬容。
    if pred_val > 1.0 and 0 < abs(truth_val) < 1.0:
        ...
        if _PERCENT_CONTEXT.search(tail):
            ...
            if abs_diff <= 0.01 or rel_ok:
                logger.debug(
                    "語意百分比修正（c 層，容差 10%%%%）：%.4f → %.6f（末尾含百分比關鍵字）",
                    pred_val, adjusted,
                )
                return True, False

    # d. 正負號容差：財務變動幅度的正負號歧義
    #    若符號相反但絕對值（含 ÷100 / ×100 換算）在容差內 → 視為正確
    if pred_val * truth_val < 0:  # 一正一負
        abs_p, abs_t = abs(pred_val), abs(truth_val)
        for candidate in (abs_p, abs_p / 100.0, abs_p * 100.0):
            if within_tolerance(candidate, abs_t):
                logger.debug(
                    "正負號容差：pred=%.4f, truth=%.4f（絕對值相符，符號歧義忽略）",
                    pred_val, truth_val,
                )
                return True, False
...
async def run_evaluation(args: argparse.Namespace) -> EvalReport:
    ...
    # 4. 動態路由：依 --backend 參數或自動判斷選擇推理後端
    #    --backend vllm     → 強制 vLLM（guided_json 模式）
    #    --backend ollama   → 強制 Ollama（重試模式）
    #    --backend auto     → 自動判斷：port 8000 → vLLM，其他 → Ollama
    top_k = args.top_k
    
    if args.backend == "auto":
        use_vllm = "8000" in args.defense_base_url
        logger.info("後端自動路由模式：檢測 URL 中的端口號…")
    else:
        use_vllm = (args.backend == "vllm")
        logger.info("後端指定模式：%s", "vLLM" if use_vllm else "Ollama")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "詳細結果輸出路徑。"
            "未指定時自動產生：results/{模型短名}_ACC{準確率}.json"
        ),
    )
    parser.add_argument(
        "--vram-gb",
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "推理後端類型。vllm=強制使用vLLM；ollama=強制使用Ollama；"
            "auto=自動判斷（port 8000→vLLM，其他→Ollama）"
        ),
    )
    parser.add_argument(
        "--verbose",
...
def _build_output_path(report: EvalReport) -> Path:
    """
    自動產生存檔路徑：results/{模型短名}_ACC{準確率}.json
    例：Qwen/Qwen3-4B-AWQ + 42.50% → results/Qwen3-4B-AWQ_ACC42.50.json
    """
    model_short = report.model.split("/")[-1]                   # 去掉 org/ 前綴
    model_short = re.sub(r'[\\/:*?"<>| ]', "_", model_short)   # 清理非法字元
    acc_str = f"{report.accuracy:.2f}"                          # 例：42.50
