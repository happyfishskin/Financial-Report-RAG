...
**離線建庫（只做一次）**：MOPS HTML → 二維寬表 CSV（30 家 × 8 季）→ 事實表線性化（每格〔列 × 值欄〕＝一筆 Fact），產出三類索引：
數值事實表（211,263 筆）、關係事實表（投資／關係人／風險事件／半導體產業鏈，32,039 節點／125,167 邊，「誰—透過什麼關係→誰或什麼」加期別、金額、持股比率、來源等屬性）、
向量庫（row-aware 切塊、cosine、bge-small-zh-v1.5、ChromaDB 30,887 chunks）。

**線上查詢（每題）**

1. **使用者提問**
2. **Fix 6 前置攔截**：題目可直接用 metadata 定位者，於 Router 之前直接作答。
3. **SLM 初判**：Qwen3-4B-AWQ 意圖路由（Router），以 JSON Schema 強制解碼輸出槽位並以 Pydantic 驗證。
4. **Python 規則覆核**，依線索分流：
   - 有明確公司、期別、科目或比率線索 → **軌道一 a. 數值事實直查**
   - 有交易雙方、關係人、持股或子公司、上下游線索 → **軌道一 b. 關係事實直查**
   - 無法明確定位，或是文字說明型問題 → **軌道二 語意向量降級軌**

**軌道一　確定性直查**

- **a. 數值事實直查**
  - 槽位正規化（Fix 1／13／14／17）：例如把「台積電今年第二季營收」轉成「台積電／114Q2／營業收入／當期欄位」。
  - 科目四段遞進比對：完全符合 → 前綴＋非 CJK 邊界 → 寬鬆子字串包含 → difflib 模糊比對（cutoff 0.7）。
  - 表別防禦：避免同一科目名稱出現在不同報表時取錯表。
  - 缺期別且為單值題 → Fix 20 預設採最新期別 114Q4，並於答案中揭露。
- **b. 關係事實直查**：以「交易人＋交易對象＋科目」定位正確交易資料（Fix 16）；用「交易雙方＋交易科目」直接在常見關係資料找，找不到時再查原始資料。題幹含「請全部列出」的多值列舉題，全列並列作答。
- **候選為 0**（未命中）→ 降級至軌道二；**同期多列衝突**（非期別歧義）→ 拒答。

**軌道二　語意向量降級軌**（觸發：Router 判定為 semantic_rag 之口語／非結構化題，或軌道一未命中）

1. Metadata 硬過濾：先依公司、期別、表格等條件縮小範圍，避免找錯公司或年度。
2. ANN Top-K 向量搜尋：在剩下的片段中找出與問題意思最接近的證據。
3. 鍵值展開：把表格內容整理成「科目：數值」格式，讓 SLM 比較容易閱讀。
4. 受限格式回答：SLM 只能依指定 JSON 格式輸出答案、來源與狀態，不能自由寫一大段內容。

**輸出**

- **答案**：回傳數值，並保留來源檔與資料列位置，讓每個答案都能回到原始財報逐列查核。
- **拒答**
  - 查無資料：既無結構化 Fact，向量 Top-K 亦低於門檻。
  - 候選 > 1：比較題缺期別時回覆「請指定期別（可選 113Q1–114Q4，共 8 期）」。
  - 同期多列衝突：回傳原訊息。
