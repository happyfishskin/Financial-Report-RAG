...
def main() -> int:
    ...
    print(f'題目 {len(items)}｜語料 {len(docs):,}｜檢索器 bge-m3（dim={emb_m3.shape[1]}）')

    rows = []; t0 = time.time()
    for i, (lab, it) in enumerate(items, 1):
        q, gold = it['question'], str(it.get('expected_answer', ''))
        qv = np.asarray(m3.encode([q], normalize_embeddings=True), dtype=np.float32)[0]
        ids = list(np.argsort(-(emb_m3 @ qv))[:B.TOP_K])
    ...
    Path(f'{out}.json').write_text(json.dumps(
        {'retriever': 'dense BAAI/bge-m3 (fp16, max_len 512)', 'suite': suite,
         'summary': s, 'results': rows}, ensure_ascii=False, indent=2), 'utf-8')
    print(f'\nbge-m3 + 27B：EM {s["em"]:.1%}（{s["exact"]}/{n}）'
